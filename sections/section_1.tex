...
\begin{frame}{Lattice regularisation}
  ...
  \tikz \node[fill=ColourDark!15, minimum height=1cm] {%
  ...
  The quark fields are placed on the \alert{lattice sites}, while the gauge fields are
  placed on the \alert{links} connecting them
...
\begin{frame}{Lattice simulations at \texorpdfstring{$\mu > 0$}{m > 0}: Lattice saturation}
  ...
  \begin{center}
    \includegraphics[width=.7\textwidth]{saturation_plot}
  \end{center}
